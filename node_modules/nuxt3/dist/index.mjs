...
import { useNuxt, resolveFiles, defineNuxtModule, addPlugin, addVitePlugin, addWebpackPlugin, addTemplate, findPath, isNuxt3, isIgnored, resolveAlias, addPluginTemplate, logger, resolvePath, resolveModule, nuxtCtx, addComponent, tryResolveModule, installModule, loadNuxtConfig, templateUtils, normalizeTemplate, compileTemplate, normalizePlugin, importModule } from '@nuxt/kit';
...
const version = "3.0.0-27491748.3186841";
...
async function initNitro(nuxt) {
  ...
  const nitroConfig = defu(_nitroConfig, {
    ...
    publicAssets: [
      ...
      ...nuxt.options._layers.map((layer) => join(layer.config.srcDir, "public")).filter((dir) => existsSync(dir)).map((dir) => ({ dir }))
    ],
    prerender: {
...
async function writeDocumentTemplate(nuxt) {
  ...
  if (contents) {
    ...
    await promises.mkdir(dirname$1(dst), { recursive: true });
